...
test(`isValidAmount Should return true if an argummant is 5.5 or "5.5"`, () => {
  const a = 5.5;
  const b = "5.5";
...
test(`isValidAmount Should return false if an argummant is 0 or NaN or negative number`, () => {
  const zero = 0;
  const notNum = NaN;
  const negativeNum = -5;
